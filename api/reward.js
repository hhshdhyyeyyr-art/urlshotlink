...
module.exports = async function handler(req, res) {
    ...
    // Vercel Environment Variables se data lega, warna directly aapke credentials use karega
    const supabaseUrl = process.env.SUPABASE_URL || 'https://hlwnbizaaabtoafzbbax.supabase.co';
    const supabaseKey = process.env.SUPABASE_SERVICE_KEY || 'sb_publishable_12kTf5tqSSw1CrVi7OFJMg_I6h0r1m9';

    if (!supabaseUrl || !supabaseKey) {
         return res.status(500).json({ error: 'Vercel ENV variables missing' });
    }

    // URL Cleanup (safeguard against extra slashes or /rest/v1/)
    ...
    try {
        ...
        // 3. Update LINK table (Clicks and Earnings)
        await supabase.from('links').update({
            clicks: (linkData.clicks || 0) + 1,
            earnings: (linkData.earnings || 0) + earn_amount
        }).eq('id', linkData.id);

        // 4. Fetch and Update USER table (Adds balance & views)
